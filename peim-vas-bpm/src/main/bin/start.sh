#!/bin/sh
SH_DIR=$(cd `dirname $0`;pwd)
java_home=env|grep "JAVA_HOME";
latestjar=`ls $SH_DIR/.. -lt | grep '.jar'| awk '{print $9}' | sed -n '1p'`
echo 'start:'$latestjar
process_exists=`ps -ef|grep $latestjar|grep -v grep|awk '{print $2}'`
if [ -n "${process_exists}" ];then
	kill -9 ${process_exists}
fi
if [ ! -n "$java_home" ]; then
        cd $SH_DIR/../;
        nohup java -jar -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/home/peim/dump/bpm.hprof $SH_DIR/../$latestjar > /dev/null 2>&1 &
	echo " application is starting.......";
        cd -;
        exit;
else
	java -version;
        cd $SH_DIR/../;
	if [ $? -eq 0]; then
		nohup java -jar $SH_DIR/../$latestjar > /dev/null 2>&1 &
	else
		echo "this system has no jdk";
                cd -;
		exit;
	fi
fi

##!/bin/sh
#export JAVA_HOME=/Users/kingdomdong/Study/openjdk-13.0.2_osx-x64_bin/jdk-13.0.2.jdk/Contents/Home
#export PATH=$JAVA_HOME/bin:$JAVA_HOME/jre/bin:$PATH
#export CLASSPATH=$JAVA_HOME/lib:$JAVA_HOME/jre/lib:$CLASSPATH
#mvn spring-boot:run