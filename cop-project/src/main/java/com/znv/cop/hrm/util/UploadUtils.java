...
public class UploadUtils {
    ...
    private String saveFile(FileItem item) {
        ...
        if (item.getSize() > maxSize) { // 检查文件大小
            ...
            try {
                ...
                /*
                 * FileOutputStream fos = new FileOutputStream(uploadFile); // 文件全在内存中 if (item.isInMemory()) {
                 * fos.write(item.get()); } else { InputStream is = item.getInputStream(); byte[] buffer = new
                 * byte[1024]; int len; while ((len = is.read(buffer)) > 0) { fos.write(buffer, 0, len); } is.close(); }
     ...
     * @param file
     * @param basePath
     * @param formItemName 目标文件名 huyang
     */
    public static void uploadImg(MultipartFile file, String basePath, String bissDir, String formItemName) {
        if (file == null) {
            return ;
        }
        // 原文件名
        ...
        // TODO 这里命名的时候，文件夹以广告机ID还是订单号之类的？
        // 如果不是以订单号作为文件夹，那么要考虑是否要删除之前的？？？
        // 目标文件名
        String fileName = formItemName + "." + suffix;
        String fileUrl = (bissDir + fileName);
        try {
            File dest = FileUtils.createFile(basePath, fileUrl);
            file.transferTo(dest);
        } catch (Exception e) {
        	  System.out.println(e);
//            com.znv.log.Logger.L.error(e.getMessage());
        }
    }
}
